...
if Config.UseCarHud then
				...
				elseif not SeatBelt then
					...
					if speedBuffer[2] and GetEntitySpeedVector(Vehicle, true).y > 1.0 and speedBuffer[1] > 15
						...
						if #seatPlayerId > 0 then
							TriggerServerEvent("Lzr_hud:server:EjectPlayer", seatPlayerId, velBuffer[2])
						end
					end
				end
			else
	...
	RegisterNetEvent('Lzr_hud:client:EjectPlayer')
	AddEventHandler('Lzr_hud:client:EjectPlayer', function(velocity)
		if not SeatBelt then
			local co = GetEntityCoords(Player)
			local fw = Fwv(Player)
			SetEntityCoords(Player, co.x + fw.x, co.y + fw.y, co.z - 0.47, true, true, true)
...
if Config.UsePlayerStats then
	...
	RegisterNetEvent("Lzr_hud:client:GetPlayerPing", function(PlayerPing)
		PlayersPing = PlayerPing
	end)

	RegisterNetEvent("Lzr_hud:client:GetPlayerMoney", function(PlayerCash, PlayerBank)
	...
	CreateThread(function()
		while true do
			...
			TriggerServerEvent("Lzr_hud:server:GetPlayerPing") -- PLAYERS PING --
			TriggerServerEvent("Lzr_hud:server:GetPlayerMoney") -- PLAYERS MONEY --
